...
def phrase_generator():
    ...
    phrase = ['Hello', 'Whats up', 'Howdy', 'Aloha']
    # get name from the entry1 field
    name = str(entry1.get())

    return random.choice(phrase) + " " + name

def phrase_display():
    # Get greetins from a phrase_generator
    greeting = phrase_generator()

    # This creates the text field with our greeting
    greeting_display = tk.Text(master=window, height=10, width=30)
    greeting_display.grid(column = 0, row = 3)
    # need tk.END to actually place the line IDK why
